classdef FeatureExtractor
    properties
        CEDDDescriptor(1,144)
    end
    methods
        function obj = FeatureExtractor()
            obj.CEDDDescriptor(:) = 0;
        end
        
        function featuresArray = getFeatures(obj,frames)
            s = length(frames);
            featuresArray = zeros(s,144);
            f = waitbar(0,'1','Name','Feature Extraction',...
                    'CreateCancelBtn','setappdata(gcbf,''canceling'',1)');
            for i=1:s
                featuresArray = [featuresArray ; obj.CEDD(frames{i})];
                ...
                value = i/s;
                waitbar(value,f, sprintf('%3.2f percent completed ',value*100));
            end
            delete(f);
        end
        
        function  CEDDVector  = CEDD(obj,ImageRGB)
            T0 = 14;
            T1 = 0.68;
            T2 = 0.98;
            T3 = 0.98;
            ...
            for y= 1:Step_Y:TemoMAX_Y
                for x= 1:Step_X:TemoMAX_X
                    ...
                    if(Compact == 0)
                        Fuzzy10BinResultTable = obj.Fuzzy10Bin(HSV(1), HSV(2), HSV(3), 2); 
                        Fuzzy24BinResultTable = obj.Fuzzy24Bin(HSV(1), HSV(2), HSV(3), Fuzzy10BinResultTable, 2); 
            
                        for i=1:T
                            for j=1:24 
                                if(Fuzzy24BinResultTable(j) > 0)
                                    obj.CEDDDescriptor(24 * Edges(i) + j) = obj.CEDDDescriptor(24 * Edges(i) + j) + Fuzzy24BinResultTable(j);
                                end
                            end
                        end
                    else
                        ...
                        for i=1:T+1
                            for j=1:10
                                if(Fuzzy10BinResultTable(j) > 0)
                                    obj.CEDDDescriptor(10 * Edges(i) + j) = obj.CEDDDescriptor(10 * Edges(i) + j) + Fuzzy10BinResultTable(j);
                                end
                            end
                        end           
                    end
            ...
            SUM = sum(obj.CEDDDescriptor);
            obj.CEDDDescriptor(1:144) = obj.CEDDDescriptor(1:144)/SUM;

            obj.CEDDDescriptor = obj.CEDDQuant(obj.CEDDDescriptor);
            
            CEDDVector = obj.CEDDDescriptor;
        end
